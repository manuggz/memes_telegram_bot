...
def help_tg(chat_id, tema_ayuda, is_debug, xml_string):
    ...
    enviar_mensaje_ayuda_comando(tema_ayuda, chat_id,xml_string)
...
def random_tg(chat_id, is_debug):
    ...
    if imagen_aleatoria:
        enviar_imagen(chat_id,imagen_aleatoria)  # ,{"inline_keyboard":[[{"text":"Random","callback_data":"ASD"}]]})
...
def procesar_comandos(update_tg,usuario,xml_strings,comando,resto_mensaje):
    ...
    if comando == "/start":
        ...
        help_tg(
            update_tg.message.chat.id,resto_mensaje,
            update_tg.is_message_debug,xml_strings
        )
    elif comando == "/random":
        ...
        stop_tg(
            update_tg.message.chat.id,usuario,
            update_tg.is_message_debug,xml_strings
        )
        ...
        imagen_enviada=sendme_tg(
            update_tg.message.chat.id,usuario,
            resto_mensaje,update_tg.is_message_debug,
            xml_strings
        )
